...
COMPOSE_FILE="cs2-classic-server-docker-compose.yml"
PROJECT="${COMPOSE_PROJECT_NAME:-cs2-classic}"
DRY_RUN=false
ALLOW_MAJOR=false
for arg in "$@"; do
  case "$arg" in
...
if [ "$new_major" != "$cur_major" ] && [ "$ALLOW_MAJOR" != "true" ]; then
  ...
  echo "  https://github.com/heyvaldemar/cs2-classic-server-docker-compose/releases/tag/$latest" >&2
  exit 3
fi
